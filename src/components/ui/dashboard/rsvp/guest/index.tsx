...
import PhoneIcon from "@/components/static/icons/phone"
import SortIcon from "@/components/static/icons/sort"
import CustomButton from "@/components/ui/Button"
import Chip from "@/components/ui/commons/Chip"
import TabView from "@/components/ui/commons/TabView"
import { getCurrentWedding } from "@/lib/cookies/currentWeddingCookie"
import { RSVP } from "@/lib/raw-data/rsvp"
import {
  useBulkDeleteGuestsMutation,
  useGetGuestListQuery,
} from "@/lib/redux/features/apis/guestApi"
import { cn } from "@/lib/utils/cn"
import { Check, FilterIcon, Loader, Trash } from "lucide-react"
...
import GuestFilterAndSortOptions from "./GuestFilterAndSortOptions"
...
import { EVENT_ICONS } from "@/components/ui/commons/InputBoxWithEmoji"
import Image from "next/image"
import { Tooltip } from "primereact/tooltip"
import NewGroupForm from "@/components/ui/forms/NewGroupForm"
import { useGetGroupsQuery } from "@/lib/redux/features/apis/groupApi"
import { GroupCard } from "@/components/ui/dashboard/rsvp/guest/GroupCard"
import { getWeddingInfo } from "@/lib/utils/weddingUtil"
import GuestsSideSheetContent from "./GuestsSideSheetContent"

const TABS = ["Individual", "Group"]

interface GuestRsvpPageProps {
  addGuestFormOpen: boolean
  setAddGuestFormOpen: (value: boolean) => void
  clearData: boolean
  setClearData: (value: boolean) => void
  addGroupFormOpen: boolean
  setAddGroupFormOpen: (value: boolean) => void
  viewData: Group | null
  setViewData: (value: Group) => void
}
...
export default function GuestRsvpPage({
  ...
  const sortTieredMenu = useRef<TieredMenu>(null)
  const [tab, setTab] = useState(TABS[0])
  const [selectedGuests, setSelectedGuests] = useState<RSVP[]>([])
  ...
  //Filters
  const [status, setStatus] = useState<null | "undecided" | "coming">(null)
  const [eventFilter, setEventFilter] = useState<any>(null)
  const [belongsToFilter, setBelongsToFilter] = useState<any>(null)
  // Sort
  const [sort, setSort] = useState<{
    type: "name"
    order: "name_asc" | "name_desc" | null
  }>({ type: "name", order: null })
  ...
  const {
    data: groups,
    isError: useGetGroupsQuery_isError,
    error: useGetGroupsQuery_error,
  } = useGetGroupsQuery(currentWeddingId)

  const { data: weddings } = useGetWeddingsQuery(undefined)
  const foundWedding = weddings && getWeddingInfo(weddings, currentWeddingId)

  const [
    bulkDelete,
    { isLoading: useBulkDeleteGuestsMutation_isLoading, isError, error },
  ] = useBulkDeleteGuestsMutation()
  ...
  const {
    ...
    pagination: {
      ...
      ...(status !== null && { rsvp: status === "coming" }),
      ...(eventFilter && { eventId: eventFilter }),
      ...(belongsToFilter && { guestBelongsTo: belongsToFilter }),
      ...(sort?.order && {
        sortDir: sort.order === "name_asc" ? "asc" : "desc",
      }),
    },
  })

  const totalItems = parseInt(data?.pagination["totalItems"] || 0, 10)

  const onPageChange = (event: any) => {
    setPageNo(event.first / pageSize + 1)
  }

  const [currentSelectedGuest, setCurrentSelectedGuest] = useState<any>()
  useEffect(() => {
    if (error) {
      toast(`Error: ${error}`)
    }
    if (useGetGuestListQuery_error) {
      toast(`Error: ${useGetGuestListQuery_error}`)
    }
    if (useGetEventsQuery_error) {
      toast(`Error: ${useGetEventsQuery_error}`)
    }
    if (useGetGroupsQuery_error) {
      toast(`Error: ${useGetGroupsQuery_error}`)
    }
  }, [
    isError,
    error,
    useGetGuestListQuery_isError,
    useGetGuestListQuery_error,
    useGetEventsQuery_isError,
    useGetEventsQuery_error,
    useGetGroupsQuery_isError,
    useGetGroupsQuery_error,
  ])
  useEffect(() => {
    if (data == null) {
      return
    ...
    const dummyRsvp: RSVP[] = data.items.map((_: any) => ({
      id: _.guest_id,
      name: _.guest_name,
      contact: {
        email: _.email,
      ...
      address: `${_.address}, ${_.city} ${_.state}, ${_.country}-${_.zip_code}`,
      rsvpStatus: _.rsvp ? "Coming" : "Undecided",
      events: _.event_list,
      relation: _.guest_belongs_to,
      plusOne: _.plus_one,
      rawData: _,
    }))
    setRsvpList([...dummyRsvp])
    const guestIds: any[] = data.items.map((item: any) => item.id)
    setSelectedGuests(
      selectedGuests.filter((item) => guestIds.includes(item.id))
    )
  }, [data])

  // useEffect(() => {
  //   if (clear) {
  //     setCurrentFocussedGroup(null)
  //   }
  // },
  //  [setClear, clear])
  ...
  const debouncedLog = useCallback(
    ...
    [] // Dependencies for useCallback (no dependencies to ensure it's created once)
  )

  useEffect(() => {
    // Call the debounced function whenever searchQuery changes
    debouncedLog(searchQuery)
  }, [searchQuery, debouncedLog])

  const contactTemplate = (rowData: RSVP) => {
    return (
      <div className="flex flex-col">
        <div className="flex items-center">
          <MessageIcon className="mr-2" />
          <p>{rowData.contact.email}</p>
        </div>
        <div className="flex items-center">
          <PhoneIcon className="mr-2" />
          <p>{rowData.contact.phone}</p>
        </div>
      </div>
    )
  }
  const addressTemplate = (rowData: RSVP) => {
    return (
      <div className="flex max-w-[80%] items-center">
        <p className="whitespace-normal">{rowData.address}</p>
      </div>
    )
  }

  const eventsTemplate = (rowData: RSVP) => {
    return (
      <div className="flex gap-2">
        {((rowData as any).events || [])
          .slice(0, 3)
          .map((event: any, index: number) => (
            <div
              key={event?.id}
              className={cn(
                "flex size-7 items-center justify-center rounded-md border-4 border-[#B6D3D6] bg-[#ECF4F5]",
                `event-${event?.id}-${index}`
              )}
            >
              <Tooltip
                target={`.event-${event?.id}-${index}`}
                position="bottom"
              >
                <div>{event?.event_name}</div>
              </Tooltip>
              <Image
                src={EVENT_ICONS[parseInt(event?.icon_id) - 1 || 0]}
                alt="Event icon"
                width={16}
                height={16}
              />
  ...
  const rsvpStatusTemplate = (rowData: RSVP) => {
    return <Chip label={rowData.rsvpStatus} />
  }

  const plusOneTemplate = (rowData: RSVP) => {
    if (rowData.plusOne) {
      return (
  ...
  const nameTemplate = (rowData: RSVP) => {
    return (
      <div className="flex flex-row items-center gap-2">
        <span className="mr-1 flex size-8 items-center justify-center rounded-full bg-[#FF2D55] text-white">
          {rowData.name[0].toUpperCase()}
        </span>
        <div className="flex flex-col gap-0.5">
          <p>{rowData.name}</p>
          <div className="w-min rounded-full border border-pink-500 px-2 py-0.5 text-xs text-gray-700">
            {rowData.relation}
          </div>
        </div>
      </div>
  ...
  function handleFilter(
    filterType: "undecided" | "coming",
    value: boolean
  ): void {
    if (!value) {
      setStatus(null)
    } else {
      setStatus(filterType)
    }
  }
  ...
  return (
    <div className="h-full">
      ...
      <div className="flex flex-col items-center justify-between px-2 py-3 md:flex-row">
        <div className="mb-2 flex w-full flex-row items-center justify-between gap-2 md:mb-0 md:w-auto md:gap-4">
          <TabView tabNames={TABS} value={tab} onChange={setTab} />
          <span className="text-center md:text-left">
            {useGetGuestListQuery_isFetching ? (
              <div className="item-center flex gap-1">
                <Loader size={20} className="animate-spin" />
                Loading
              </div>
            ) : (
              `Result: ${pageNo} out of ${Math.ceil(totalItems / pageSize) || 1}`
            )}
          </span>
        </div>
        <div className="flex w-full flex-wrap justify-end gap-2 md:w-auto">
            ...
            model={[
              {
                ...
                items: [
                  {
                    label: "Coming",
                    command: () => handleFilter("coming", status == null),
                    icon: (
                      <div className="mr-4 size-3 rounded-full bg-green-500" />
                    ),
                    className: status === "coming" ? "bg-gray-100" : "",
                  },
                  {
                    label: "Undecided",
                    command: () => handleFilter("undecided", status == null),
                    icon: (
                      <div className="mr-4 size-3 rounded-full bg-gray-500" />
                    ),
                    className: status === "undecided" ? "bg-gray-100" : "",
                  },
                ],
              },
              {
                ...
                items:
                  eventsData?.events.map((event: any) => ({
                    ...
                    command: () => {
                      if (event.event_id === eventFilter?.event_id) {
                        setEventFilter(null)
                      } else {
                        setEventFilter(event.event_id)
                      }
                    },
                    className:
                      eventFilter?.event_id === event.event_id
                        ? "bg-gray-100"
                        : "",
                    icon: <div className="mr-2">{event?.event_icon_id}</div>,
                  })) || [],
              },
              {
                label: "Belongs to",
                items: foundWedding
                  ? foundWedding.couple?.map((person: any) => ({
                      label: `${person.first_name} ${person.last_name}`,
                      value: `${person.first_name} ${person.last_name}`,
                      command: () => {
                        const newVal = `${person.first_name} ${person.last_name}`
                        if (newVal === belongsToFilter) {
                          setBelongsToFilter(null)
                        } else {
                          setBelongsToFilter(newVal)
                        }
                      },
                    }))
                  : [],
          ...
          <CustomButton
            size="icon"
            variant="ghost"
            onClick={(e) => sortTieredMenu.current?.toggle(e)}
          >
            <SortIcon />
          </CustomButton>
          <TieredMenu
                ...
                items: [
                  {
                    ...
                    command: () =>
                      setSort((prev) => ({
                        type: "name",
                        order:
                          prev.type !== "name" || prev.order !== "name_asc"
                            ? "name_asc"
                            : null,
                      })),
                    className:
                      sort?.type === "name" && sort.order === "name_asc"
                        ? "bg-gray-100"
                        : "",
                  },
                  {
                    ...
                    command: () =>
                      setSort((prev) => ({
                        type: "name",
                        order:
                          prev.type !== "name" || prev.order !== "name_desc"
                            ? "name_desc"
                            : null,
                      })),
                    className:
                      sort?.type === "name" && sort.order === "name_desc"
                        ? "bg-gray-100"
                        : "",
                  },
                ],
          ...
          <UploadCsvComponent weddingId={currentWeddingId} />
        </div>
      </div>
      {tab === "Individual" ? (
        <div className="h-full whitespace-nowrap">
          <GuestFilterAndSortOptions
            status={status}
            setStatus={setStatus}
            eventFilter={eventFilter}
            setEventFilter={setEventFilter}
            belongsToFilter={belongsToFilter}
            setBelongsToFilter={setBelongsToFilter}
            sort={sort}
            setSort={setSort}
          />
          ...
          <DataTable
            value={
              useGetGuestListQuery_isFetching ? Array(5).fill({}) : rsvpList
            }
            responsiveLayout="scroll"
            selection={selectedGuests}
            emptyMessage="No guests available"
            onSelectionChange={(e) => {
              if (useGetGuestListQuery_isFetching) {
                return
              }
              setSelectedGuests(e.value)
            }}
            selectionMode="checkbox"
            disabled={useGetGuestListQuery_isFetching}
            onRowClick={(e) => {
              if (useGetGuestListQuery_isFetching) {
                return
              }
              setClearData(false)
              setCurrentSelectedGuest(e.data)
            ...
            footer={
              ...
              (data == undefined ||
                data.items == undefined ||
                data.items.length == 0) && <EmptyStateTemplate />
            }
            onPage={onPageChange}
          >
          ...
          {groups && groups.groups && groups?.groups.length > 0 ? (
            groups?.groups?.map((group: Group) => (
              <div
                key={group.group_id}
                className={cn(
                  "mb-2 flex w-full cursor-pointer items-center rounded-md p-2"
                )}
              >
                <GroupCard
                  group={group}
      ...
      <Sidebar
        position="right"
        visible={addGuestFormOpen}
        onHide={() => setAddGuestFormOpen(false)}
        className="max-width-screen w-[450px]"
        content={
          <GuestsSideSheetContent
            data={clearData ? null : currentSelectedGuest}
            onClose={() => {
              setAddGuestFormOpen(false)
            }}
          />
        }
      />
      <Sidebar
        position="right"
        visible={addGroupFormOpen}
        onHide={() => setAddGroupFormOpen(false)}
        className="max-width-screen w-[450px]"
        content={
          <NewGroupForm
            // viewData={clearData ? null : currentSelectedGuest}
            viewData={viewData}

...
import { useGetSeatingsQuery } from "@/lib/redux/features/apis/seatingApi"
import { useAppSelector } from "@/lib/redux/hooks"
import { getCurrentWedding } from "@/lib/cookies/currentWeddingCookie"
import { toast } from "sonner"
import SeatingSidesheetContent from "./SeatingSidesheetContent"

interface RsvpSeatingProps {
  visible: boolean
...
const SeatingDashboard: React.FC<RsvpSeatingProps> = (props) => {
  ...
  const { data, isLoading, isError, error } = useGetSeatingsQuery({
    weddingId: currentWeddingId,
  })
  useEffect(() => {
    if (isError) {
      console.error("Error fetching seatings:", error)
      toast("Failed to fetch seating arrangements. Please try again later.")
    }
  }, [isError, error])
  if (isLoading) {
    return (
      <div className="py-4">
        <div className="mb-6 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <Skeleton width="120px" height="2.5rem" borderRadius="2rem" />
            <Skeleton width="120px" height="2.5rem" borderRadius="2rem" />
          </div>
        </div>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {Array(8)
            ...
            .map((_, index) => (
              <Skeleton
                key={index}
                className="min-h-[250px] w-full max-w-xs overflow-hidden rounded-2xl border bg-white hover:shadow-md"
              >
                <div className="space-y-2 p-3">
                  <Skeleton className="aspect-[4/3] min-h-[150px] cursor-pointer rounded-lg object-cover" />
    ...
    return (
      <div className="flex h-full flex-col items-center justify-center">
        ...
        <p className="text-gray-500">
          No seating arrangements found. Please add a new seating arrangement.
        </p>
        <Sidebar
          position="right"
          ...
          content={
            <NewSeatingForm
              data={props.clear ? null : currentFocussedEvent}
              onClose={() => {
  ...
  return (
    <>
      ...
      <div className="seating-dashboard grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        {data?.data.seatings?.map((seating: any, index: number) => (
          <SeatingCard
            key={index}
            seating={seating}
            index={index}
            onClick={(seating) => {
              setCurrentFocussedEvent(seating)
